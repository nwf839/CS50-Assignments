...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            const char *space2 = strchr(needle - 10, ' ');
            if (*(needle - 9) != ' ')
            {
                space2 = NULL;
                error(400);
            ...
            int spaceCount = 0;
            for (int i = 0; i < (needle - haystack + 2); i++)
            {
                if (line[i] == ' ')
                {
                    spaceCount += 1;
            ...
            if (spaceCount != 2 || *needle != '\r' || *(needle + 1) != '\n')
            {
                error(400);
                continue;
            }
            ...
            {
                strncpy(absolute_path, targetStart, queryStart - targetStart);
                absolute_path[queryStart - targetStart] = '\0';
                
                strncpy(query, queryStart + 1, targetEnd - queryStart);
                query[space2 - queryStart] = '\0';
            }

            char http_version[needle + 2 - space2];
            strncpy(http_version, space2 + 1, needle - space2 + 1);
            http_version[needle - space2 + 1] = '\0';
            ...
            const char *extensionStart = strchr(targetStart, '.') + 1;
            char extension[targetEnd - targetStart];
            if (queryStart == NULL)
            {
                strncpy(extension, extensionStart, targetEnd - extensionStart + 1);
                extension[targetEnd - extensionStart + 1] = '\0';
            }
            else
            {
